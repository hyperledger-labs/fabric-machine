...
func preprocessProtoBlock(txMgr txmgr.TxMgr,
	...
	for txIndex, envBytes := range block.Data.Data {
		...
		// TODO(harisj): Figure out a way to skip proto unmarshalling when hardware is used.
		...
		if txsFilter.IsInvalid(txIndex) {
			...
			// logger.Warningf("Block [%d] Transaction index [%d] marked as invalid by committer. Reason code [%s]",
			// 	block.Header.Number, txIndex, txsFilter.Flag(txIndex).String())
			continue
		}
		if err != nil {
			return nil, nil, err
		...
		if txRWSet != nil {
			...
			if err := validateWriteset(txRWSet, validateKVFunc); err != nil {
				...
				// logger.Warningf("Block [%d] Transaction index [%d] marked as invalid. Reason code [%s]",
				// 	block.Header.Number, txIndex, peer.TxValidationCode_INVALID_WRITESET)
