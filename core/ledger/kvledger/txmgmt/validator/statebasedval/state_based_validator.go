...
func (v *Validator) ValidateAndPrepareBatch(block *internal.Block, doMVCCValidation bool) (*internal.PubAndHashUpdates, error) {
	...
	if block.Num == 0 || !hwEnabled {
		...
		// here.
		fmBlock, err := fabmac.GetBlockData(block.Num, 0)
		if err != nil {
			return nil, err
		}
		if !fmBlock.Valid {
			logger.Warningf("Block [%d] from hardware is invalid", block.Num)
			return nil, fmt.Errorf("Block [%d] is invalid", block.Num)
		}
