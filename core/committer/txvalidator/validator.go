...
func (v *TxValidator) Validate(block *common.Block) error {
	...
	logger.Infof("[%s] START Block Validation for block [%d]", v.ChainID, block.Header.Number)
	...
	elapsedValidation := time.Since(startValidation) / time.Microsecond // duration in us
	logger.Infof("[%s] Validated block [%d] in %dus", v.ChainID, block.Header.Number, elapsedValidation)
...
func (v *TxValidator) validateTx(req *blockValidationRequest, results chan<- *blockValidationResult) {
	...
	if env, err := utils.GetEnvelopeFromBlock(d); err != nil {
		...
		if common.HeaderType(chdr.Type) == common.HeaderType_ENDORSER_TRANSACTION {
			...
			// Skip vscc when hardware is used since its handled in hardware.
			if !hwEnabled {
		...
		// Skip proto marshalling when hardware is used since its handled in hardware.
		if !hwEnabled {
