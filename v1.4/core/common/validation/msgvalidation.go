...
func ValidateTransaction(e *common.Envelope, c channelconfig.ApplicationCapabilities) (*common.Payload, pb.TxValidationCode) {
	...
	// Skip tx signature check when hardware is used since its handled in hardware.
	if !hwEnabled {
	...
	switch common.HeaderType(chdr.Type) {
	case common.HeaderType_ENDORSER_TRANSACTION:
		// When hardware is used, we skip all the tx validation logic since its handled in
		// hardware. Here we assume the tx to be valid, and later merge in the actual validation
