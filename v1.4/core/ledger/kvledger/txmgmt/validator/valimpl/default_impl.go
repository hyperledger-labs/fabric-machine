...
func InitFabricMachine() error {
	return statebasedval.InitFabricMachine()
}

func CloseFabricMachine() {
	statebasedval.CloseFabricMachine()
}
...
func (impl *DefaultImpl) ValidateAndPrepareBatch(blockAndPvtdata *ledger.BlockAndPvtData,
	...
	logger.Infof("ValidateAndPrepareBatch() for block number = [%d]", block.Header.Number)
	...
	logger.Info("ValidateAndPrepareBatch() complete")
