...
import (
	...
	"github.com/hyperledger/fabric/common/bcm"
...
func (h *Handler) deliverBlocks(ctx context.Context, srv *Server, envelope *cb.Envelope) (status cb.Status, err error) {
	...
	for {
		...
		// send block to hardware using bcm protocol
		bcm.SendToHardware(ctx, block)
		...
		logger.Debugf("[channel: %s] Done delivering block [%d] for (%p) for %s", chdr.ChannelId, block.Header.Number, seekInfo, addr)
